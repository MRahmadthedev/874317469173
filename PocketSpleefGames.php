...
name=PocketSpleefGames
description=Spleef games plugin.
version=1.0
apiversion=10
author=codmadnesspro
class=PocketSpleefGames
*/
...
class PocketSpleefGames implements Plugin{
	...
	public function init() {
		...
		$this->api->console->register("sg", "PocketSpleefGames command.", array($this, "command"));
		$this->api->console->register("kit", "PocketSpleefGames command.", array($this, "command"));
		$this->api->ban->cmdWhitelist("kit");
		$this->api->ban->cmdWhitelist("sg");
	}
	...
	public function handle(&$data, $event) {
		switch ($event) {
			...
			case "player.spawn":
				$this->api->console->run("give ".$data->username." diamond_pickaxe")
			...
			case "player.death":
				...
				if (is_numeric($data["cause"])) {
					...
					if ($entity instanceof Entity and $entity->class === ENTITY_PLAYER) {
						$er = $entity->name;
						if (!isset($this->players[$er])) {
							$this->kick($er, "unknown", false, false);
							break;
						}
						$this->score[$dead]["cause"] = $er;
						$this->score[$er][""][] = $dead;
						$reason = "ed by $er!";
						$this->giveEXP($er, $this->config["exp"][""]);
						$point = $this->kit->grantPocketCash($dead);
						$this->kit->grantPocketCash($er, $point);
					} else {
						$reason = "ed.";
						$this->score[$dead]["cause"] = "ed";
					}
				} else {
					$reason = "accident.";
				...
				if ($nop > 1) {
					$this->broadcast(FORMAT_AQUA."[SpleefGames] ".$nop." players remaining.");
				} else {
...
Spleef
			...
			case "sg":
				...
				switch($mode) {
					case "run":
						...
						console("");
						$this->api->chat->broadcast(FORMAT_AQUA."[SpleefGames] SpleefGame server has been running!!");
						console("");
					...
					case "start":
						if ($this->status !== "lobby") {
							$output .= "[SpleefGames] This command is unavailable now.\n";
							break;
						}
					...
					case "stop":
						if ($this->status === false) {
							$output .= FORMAT_YELLOW."[SpleefGames] game is not opened!\n";
							break;
						}
						$this->gameStop();
						break;
					case "marker":
						...
						if (!$this->formatBool($bool)) {
							$output .= "Usage: /sg marker <on | off>\n";
							break;
						}
						if ($bool) {
							$this->placePointMarker();
							$output .= "[SpleefGames] placed markers on the world!";
						} else {
							$this->breakPointMarker();
							$output .= "[SpleefGames] breaked markers on the world!";
						}
						break;
					case "state":
					case "status":
					...
					case "settime":
						...
						if (empty($time) or $time <= 0 or !in_array($mode, array("lobby", "invincible", "play"))) {
							$output .= "Usage: /sg settime <status> <time(sec)>\n<status> ... \"ready\" or \"invincible\" or \"play\"\n";
							break;
						}
					...
					case "locktime":
						...
						if (in_array($mode, array("day", "night", "sunset", "sunrise"))) {
							...
							$this->writeConfig();
							$output .= "[SpleefGames] Setted lock-time to \"$mode\".\n";
						} else {
							$output .= "[SpleefGames] Failed to set lock-time.\n";
						}
						break;
					case "setprize":
						$amount = (int) array_shift($params);
						if ($amount = "" or $amount < 0) {
							$output .= "Usgae :/sg setprize <amount>\n";
							break;
						}
					...
					case "protect":
						$bool = array_shift($params);
						if (!$this->formatBool($bool)) {
							$output .= "[SpleefGames] Usage: /sg protect <on | off>\n";
							break;
						}
						if ($bool) {
							$output .= "[SpleefGames] Turned on world protection for tournaments.\n";
							$this->config["world-protect"] = true;
						} else {
							$output .= "[SpleefGames] Turned off the protection for tournaments.\n";
					...
					case "addfield":
						...
						if (empty($field)) {
							$output .= "[SpleefGames] Usage: /sg addfield <field name>\n";
							break;
						}
						if ($this->isAlnum($field) === false) {
							$output .= FORMAT_YELLOW."[SpleefGames] You need to use English for field name.";
							break;
						}
						if (empty($levelname)) {
							$levelname = false;
						...
						if ($this->fieldExists($field)) {
							$output .= FORMAT_YELLOW."[SpleefGames] \"".$field."\" already exists!\n";
							break;
						}
						$this->config["field"][$field] = array("lobby" => array(), "start" => array(), "level" => $levelname);
						$this->writeConfig();
						$output .= FORMAT_AQUA."[SpleefGames] Adding \"".$field."\" field succeeded! Next, you must set a lobby point and start points of the field.\n";
						$output .= "[SpleefGames] Usage: /sg setlobby <field name> <x> <y> <z>\n";
						$output .= "[SpleefGames] Usage: /sg addpoint <field name> <x> <y> <z>\n";
						$output .= "[SpleefGames] Usage: /sg rmpoint <field name> <number>\n";
						break;
					case "rmfield":
						$field = (string) array_shift($params);
						if (!$this->fieldExists($field)) {
							$output .= FORMAT_YELLOW."[SpleefGames] \"".$field."\" doesn't exist!\n";
							break;
						}
						unset($this->config["field"][$field]);
						$output .= FORMAT_AQUA."[SpleefGames] Removing \"".$field."\" field succeeded!\n";
						$this->writeConfig();
						break;
					case "fieldinfo":
						$field = (string) array_shift($params);
					...
					case "debug":
						...
						break;
					default:
						$output .= "Usage: /sg run ...run SpleefGames\n";
						$output .= "Usage: /sg start ...start a game\n";
						$output .= "Usage: /sg stop ...suspend the game\n";
						$output .= "Usage: /sg marker <on | off>\n";
						$output .= "Usage: /sg addfield <field name>\n";
						$output .= "Usage: /sg rmfield <field name>\n";
						$output .= "Usage: /sg fieldinfo <field name>\n";
						$output .= "Usage: /sg setlobby <field name> <x> <y> <z>\n";
						$output .= "Usage: /sg setprize <amount>\n";
						$output .= "Usage: /sg protect <on | off>\n";
						$output .= "Usage: /sg addpoint <field name> <x> <y> <z>\n";
						$output .= "Usage: /sg rmpoint <field name> <number>\n";
				}
				break;
			case "kit":
				$mode = array_shift($params);
				switch ($mode) {
					...
					case "add":
						...
						if ($this->kit->add($name, $price, $level) === true) {
							$output .= FORMAT_DARK_AQUA."[SpleefGames] Added $name kit!\n";
						} else {
							$output .= FORMAT_YELLOW."[SpleefGames] Failed to add $name !\n";
						}
						break;
					case "removekit":
					case "remove":
					...
					case "rm":
						...
						if ($this->isAlnum($kitname) === false) {
							$output .= FORMAT_YELLOW."[SpleefGames] You need to use English for kit name.";
							break;
						}
						if ($this->kit->remove($kitname)) {
							$output .= FORMAT_DARK_AQUA."[SpleefGames] Removed \"$kitname\"!\n";
						} else {
							$output .= FORMAT_YELLOW."[SpleefGames] Failed to remove \"$kitname\"!\n";
						}
						break;
					case "list":
					...
					case "additem":
						...
						if ($this->kit->get($kitname) === false) {
							$output .= FORMAT_YELLOW."[SpleefGames] The kit \"$kitname\" doesn't exist.\n";
							break;
						}
						if (!isset(Block::$class[$id]) and !isset(Item::$class[$id])) {
							$output .= FORMAT_YELLOW."[SpleefGames]NOTICE: The item id \"$id\" could be incorrect.\n";
						}
						if ($meta === null) {
							$meta = 0;
						}
						...
						if ($this->kit->editItem("add", $kitname, $sets)) {
							$output .= FORMAT_DARK_AQUA."[SpleefGames] Added items to \"$kitname\"!\n";
						} else {
							$output .= FORMAT_YELLOW."[SpleefGames] Failed to add items to \"$kitname\".\n";
						}
						$this->kit->showKitInfo($kitname);
						break;
					case debug:
	...
	public function playerCommand($cmd, $params, $issuer, $alias) {
		...
		switch ($cmd) {
			case "kit":
				...
				break;
			case "sg":
				$this->showAccountInfo($issuer);Spleef
				break;
		}
		return $output;
	}
	...
	public function editField($mode, $params, &$output) {
		$field = array_shift($params);
		if (!$this->fieldExists($field)) {
			$output .= FORMAT_YELLOW."[SpleefGames] The field doesn't exist!".FORMAT_RESET.": \"".FORMAT_GREEN."".$field.FORMAT_RESET."\"\n";
			$output .= "[SpleefGames] Usage: /sg setlobby <field name> <x> <y> <z>\n";
			$output .= "[SpleefGames] Usage: /sg addpoint <field name> <x> <y> <z>\n";
			$output .= "[SpleefGames] Usage: /sg rmpoint <field name> <number>\n";
			return;
		...
		switch ($mode) {
			case "setlobby":
				...
				$this->writeConfig();
				$output .= FORMAT_AQUA."[SpleefGames] Setted!\n";
				$this->showFieldInfo($field);
				break;
			case "addpoint":
				$x = array_shift($params);
				...
				$z = array_shift($params);
				if (!is_numeric($x) or !is_numeric($y) or !is_numeric($z)) {
					$output .= "[SpleefGames] Usage: /sg addlobby <field name> <x> <y> <z>\n";
					break;
				}
				$x = (float) $x;
				$y = (float) $y;
				...
				$this->writeConfig();
				$output .= FORMAT_AQUA."[SpleefGames] Added!\n";
				$this->showFieldInfo($field);
				break;
			case "rmpoint":
				...
				if (!isset($this->config["field"][$field]["start"][$number])) {
					$output .= FORMAT_YELLOW."[SpleefGames] No.".$number." has not been setted!\n";
					break;
				}
				unset($this->config["field"][$field]["start"][$number]);
				$this->config["field"][$field]["start"] = array_values($this->config["field"][$field]["start"]);
				$this->writeConfig();
				$output .= FORMAT_AQUA."[SpleefGames] Removed No.\"".$number."\" point!\n";
				$this->showFieldInfo($field);
				break;
		}
	}
	...
	public function gameInvincible() {
		...
		if ($this->setPlayers() === false) {
			$this->broadcast(FORMAT_YELLOW."[SpleefGames] Failed to start a tournament!");
			$this->broadcast(FORMAT_YELLOW."[SpleefGames] It requires 2 or more people.");
			console("");
			$this->gameSuspend();
			return;
		}
		...
		$this->broadcast(" ");
		$this->broadcast(FORMAT_YELLOW."The spleef games have started!");
	...
	public function gameFinish($winner = false) {
		...
		if ($winner !== false) {
			...
			$this->broadcast(FORMAT_YELLOW."================================================");
			$this->broadcast(" ");
			$this->broadcast(FORMAT_YELLOW."[SpleefGames] This game is suspended!");
			$this->broadcast(FORMAT_YELLOW."[SpleefGames] Remnants list:");
			$this->broadcast(FORMAT_YELLOW.$message);
			$this->broadcast(" ");
			$this->broadcast(FORMAT_YELLOW."================================================");
		}
		$this->schedule(5, "broadcast", "[SpleefGames] All players will be kicked.");
	...
	public function gameStop() {
		$this->broadcast(FORMAT_RED."[SpleefGames] Stopped working by the host!");
		$this->cancelAllSchedules();
		$this->resetParams();
	}
	...
	public function tool($tool, $params) {
		switch($tool) {
			...
			case "backup":
				...
				if ($bool) {
					console("[SpleefGames] The world data is backuped.");
					$this->backupChest();
				} else {
					$this->restoreWorld();
				}
	...
	public function setPlayers() {
		...
		if (count($players) !== 0) {
			console("");
			console(FORMAT_AQUA."[SpleefGames] Player list:");
			console();
			foreach ($players as $player) {
				if ($player->gamemode == 0) { //creative
					...
					$player->sendChat("[SpleefGames] You are not Survival mode!");
					$player->sendChat("[SpleefGames] You will be kicked.");
	...
	public function setField($field = false) {
		if (count($this->config["field"]) == 0) {
			console(FORMAT_YELLOW."[SpleefGames] There is no field data! You have to add a field at first.");
			console ("[SpleefGames] Usage: /sg addfield <field name>");
			return false;
		}
		if ($field === false) {
			...
			if ($field === false) {
				console(FORMAT_YELLOW."[SpleefGames] There is no proper field data! You have to add a field at first.");
				console ("[SpleefGames] Usage: /sg addfield <field name>");
				return false;
			}
			...
			console(FORMAT_YELLOW."[SpleefGames] \"".$field."\" doesn't exist!");
			return false;
		}
		if (!$this->testField($field)) {
			console(FORMAT_YELLOW."[SpleefGames] \"".$field."\" has some incomplete parts!");
			return false;
		}
		$this->broadcast("[SpleefGames] Next stage is selected to \"".$field."\"!");
		return true;
	}
	...
	public function readyScore() {
		...
		foreach ($this->players as $player) {
			$this->score[$player->username] = array("" => array(), "cause" => " - ", "exp" => (int) 0);
		}
	}
	...
	public function getFieldLevel() {
		...
		if ($level === false) {
			console(FORMAT_YELLOW."[SpleefGames] level: \"".$field."\" doesn't exist!");
			$this->gameStop();
		}
		return $level;
	}
	...
	public function showState(&$output) {
		$output .= FORMAT_AQUA."[SpleefGames] The tournament's state:".FORMAT_RESET."   [Remnants: ".FORMAT_GREEN."".count($this->players).FORMAT_RESET."/".FORMAT_GREEN."".count($this->score).FORMAT_RESET." players]\n";
		foreach ($this->score as $user => $score) {
			...
			$output .= $user."   ".FORMAT_RESET." death: ".$score["cause"]."]\n";
		}
	}
	...
	public function showRecords(&$output) {
		...
		foreach ($records as $rec) {
			$d = $this->dFormula($rec["death"]);
			$name = substr($rec["username"], 0, 15);
			$name = $name . str_repeat(" ", 15 - strlen($name));
			$output .= $name."| level:".FORMAT_AQUA.$rec["level"].FORMAT_RESET." exp:".FORMAT_LIGHT_PURPLE.$rec["exp"].FORMAT_RESET." playing:".FORMAT_GREEN.$rec["times"].FORMAT_RESET." win:".FORMAT_DARK_AQUA.$rec["win"].FORMAT_RESET." :".FORMAT_YELLOW.$rec[""].FORMAT_RESET." death:".FORMAT_RED.$rec["death"].FORMAT_RESET." k/d:".FORMAT_BLUE.$d.FORMAT_RESET."\n";
		}
	}
	...
	public function placePointMarker() {
		...
		foreach ($this->config["field"] as $field => $data) {
			if ($data["level"] === false) {
				...
				if ($level === false) {
					console(FORMAT_YELLOW."[SpleefGames] ".$field."'s level doesn't exist!".FORMAT_RESET."");
					continue;
				}
			}
			$line1 = "Field: " . $field;
	...
	private function dFormula($, $death) {
		if ($ === 0 and $death === 0) {
			$d = " - ";
		} elseif ($ === 0) {
			$d = (String) "0.00";
	...
	private function record($winner, $scores) {
		...
		$stmt = $this->db->prepare("UPDATE records SET times = times + 1, exp = exp + :exp,  =  + :, lose = lose + 1, death = death + 1, lastjoin = datetime ('now', 'localtime') WHERE username = :username");
...
class KitManager{
	private $api, $config, $kitdb, $point, $players;
	...
	public function buy($user, $level, $kitname) {
		...
		$point = $this->grantPocketCash($user);
		$kit = $this->get($kitname);
		if ($kit === false) {
			$output .= "The kit \"$kitname\" doesn't exist!\n";
		} else if ($point < $kit["price"]) {
			$output .= "You are short of point to buy this kit!\n";
			$output .= "point: $point,  price: ".$kit["price"]."\n";
	...
	public function add($name, $price, $level) {
		...
		if (isset($sets["s"][$i]) and $this->getS($sets["s"][$i]) === false) {
		$this->kitdb->exec("UPDATE kits SET s'".$i."' = '".$sets["s"][$i]."' WHERE name = '".$name."';");
		}
		}
		*/
		return true;
	...
	public function editItem($mode, $param, $sets) {
		switch ($mode) {
			case "add":
				...
				console(FORMAT_RED."[SpleefGames] cannot add items anymore.");
				return false;
			case "remove":
				$slot = (Int) $param;
				break;
	...
	public function grantPocketCash($user, $point = 0) {
		if (!array_key_exists($user, $this->point)) {
			$this->point[$user] = DEFAULT_POINT;
		}
	...
	public function showKitInfo($kitname) {
		$kit = $this->get($kitname);
		if ($kit === false) {
			console(FORMAT_YELLOW."[SpleefGames] The kit \"$kitname\" doesn't exist!");
		}
		console("");
	...
	public function DB($path) {
		...
		$this->kitdb->exec(
				...
				s0 INTEGER, s1 INTEGER, s2 INTEGER, s3 INTEGER, s4 INTEGER
		)"
		);
	}
